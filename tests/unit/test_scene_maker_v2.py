...
from src.scene_maker import (
    make_scenes, 
    generate_scene_points, 
    build_prompt, 
    call_llm, 
    parse_scene_yaml,
    TEMP_SCENE
...
class TestSceneMakerV2:
    ...
    def test_vector_store_metadata_storage(self, mock_vector_store_class):
        ...
        with patch('src.scene_maker.call_llm') as mock_llm, \
            ...
            mock_parse.return_value = [
                {"idx": 1, "pov": "main", "purpose": "test1", "tags": ["char1"], "desc": "desc1"},
                {"idx": 2, "pov": "side", "purpose": "test2", "tags": ["char2"], "desc": "desc2"},
            ]
            
            dummy_beat = {"idx": 2, "summary": "Test Beat for VectorStore", "anchor": False}
            scenes = make_scenes(dummy_beat)
            ...
            for call in calls:
                ...
                required_metadata_fields = ["beat_id", "scene_idx", "pov", "purpose", "tags"]
