...
def run_with_retry(func, *args, max_retry=2, **kwargs) -> Any:
    ...
    **kwargs : dict
    ...
    for attempt in range(max_retry + 1):
        try:
            logger.info(f"Retry Controller: Executing {func_name} (attempt {attempt + 1}/{max_retry + 1})")
            ...
            if attempt == max_retry:
                ...
                logger.error(f"Retry Controller: {func_name} failed after {max_retry + 1} attempts: {combined_message}")
